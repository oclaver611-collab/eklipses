...
async function playLoop(mySession) {
  while (stepIndex < currentScript.length) {
    ...
    if (mySession !== session) return;
    // Short dramatic lines (≤6 words) get a tight gap — keeps cinematic pacing
    const wordCount = line.text.trim().split(/\s+/).length;
    await pause(wordCount <= 6 ? 150 : 400);
    stepIndex++;
  }

  if (!isPractice) renderAskToPractice(mySession);
  else {
    // Cold open scenarios have no script steps — jump straight to free conversation
    const sc = SCENARIOS[currentScenarioKey] || {};
    if (sc.coldOpen && conversationHistory.length === 0) {
      await startFreeConversation(mySession);
    } else {
...
/* ===== Free Conversation (cold open + post-script) ===== */
async function startFreeConversation(mySession) {
  if (mySession !== session) return;

  const sc = SCENARIOS[currentScenarioKey] || {};
  const isColdOpen = !!sc.coldOpen;
...
async function runFreeConversationLoop(mySession, isColdOpen) {
  const SESSION_MS = 10 * 60 * 1000;
  const NUDGE_AT_MS = 8 * 60 * 1000;
  const SILENCE_RESCUE_MS = isColdOpen ? 9000 : 12000;
  ...
  while (mySession === session) {
    ...
    if (!nudgeDone && elapsed >= NUDGE_AT_MS) {
      nudgeDone = true;
  ...
  const sc = SCENARIOS[currentScenarioKey] || {};
  if (sc.coldOpen) {
    await speak("And... scene. Let's see how you did.", 'Ryan');
  } else {
    await speak("That's time. Good work — let's see how you did.", 'Ryan');
  }
  hideFreeConvTimer();
  await runCoachFeedback(mySession);
}
...
function showFreeConvTimer(mySession, totalSeconds) {
  let remaining = totalSeconds;
  const el = document.getElementById('freeConvTimer') || (() => {
    const d = document.createElement('div');
    d.id = 'freeConvTimer';
    d.style.cssText = 'position:fixed;top:16px;right:16px;background:rgba(0,0,0,0.7);color:#fff;padding:6px 14px;border-radius:999px;font-size:14px;font-weight:700;z-index:9999';
    document.body.appendChild(d);
    return d;
  })();
  if (freeConvTimerInterval) clearInterval(freeConvTimerInterval);
  const update = () => {
    if (mySession !== session) { clearInterval(freeConvTimerInterval); return; }
    const m = Math.floor(remaining / 60);
    const s = remaining % 60;
    el.textContent = m + ':' + String(s).padStart(2,'0');
    el.style.color = remaining <= 120 ? '#ffb300' : '#fff';
    if (remaining <= 0) { clearInterval(freeConvTimerInterval); el.style.display = 'none'; return; }
    remaining--;
  };
  update();
  freeConvTimerInterval = setInterval(update, 1000);
}

function hideFreeConvTimer() {
  if (freeConvTimerInterval) clearInterval(freeConvTimerInterval);
  const el = document.getElementById('freeConvTimer');
    ...
    <div style="display:grid;gap:10px;margin-bottom:14px">
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
        <div style="background:#162016;border:1px solid #1e3a1e;border-radius:10px;padding:12px">
          <div style="color:#40c770;font-size:11px;font-weight:700;margin-bottom:5px;text-transform:uppercase;letter-spacing:.05em">Best moment</div>
        ...
        <div style="background:#1e1616;border:1px solid #3a1e1e;border-radius:10px;padding:12px">
          <div style="color:#ff6b6b;font-size:11px;font-weight:700;margin-bottom:5px;text-transform:uppercase;letter-spacing:.05em">Missed opportunity</div>
      ...
      <div style="background:#1a1730;border:1px solid #2e2a50;border-radius:10px;padding:12px">
        <div style="color:#a78bfa;font-size:11px;font-weight:700;margin-bottom:5px;text-transform:uppercase;letter-spacing:.05em">Try this next time</div>
      ...
      <div style="background:#1a1620;border:1px solid #2e1e3a;border-radius:10px;padding:12px;display:flex;align-items:center;gap:12px">
        ...
        <div>
          <div style="color:#d4a8ff;font-size:11px;font-weight:700;margin-bottom:3px;text-transform:uppercase;letter-spacing:.05em">Would Sofia date you?</div>
