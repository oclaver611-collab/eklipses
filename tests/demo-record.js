...
//   - FISH_AUDIO_API_KEY and GROQ_API_KEY in .env
//
// Real STT path (no mock):
//   1. AI user reply generated by Groq
//   2. Text → MP3 via Fish Audio TTS (Ryan voice)
...
const RYAN_VOICE_ID   = '44b996214285427697767cb469793647';  // Ryan — only male Fish Audio voice
...
if (!FISH_AUDIO_KEY) { console.error('[DEMO] FISH_AUDIO_API_KEY not set — real STT requires TTS audio'); process.exit(1); }
...
// ── Fish Audio TTS for AI user lines (Ryan voice) ──────────────────────────────
async function fishAudioTTS(text, voiceId = RYAN_VOICE_ID) {
...
async function main() {
  ...
  const transcriptLines = [
    ...
    `URL: ${LIVE_URL}`,
    `AI user TTS: Fish Audio Ryan (${RYAN_VOICE_ID})`,
  ...
  while (Date.now() - sessionStart < SESSION_MS && turn < MAX_TURNS) {
    ...
    // Fetch TTS audio (Ryan voice) — real MP3 bytes for VB-CABLE playback
    let audioBuf = null;
    try {
      audioBuf = await fishAudioTTS(userReply);
      console.log(`[T${turn} @${elapsedMin}m] USER (TTS ${audioBuf.length}b): ${userReply}`);
    } catch (err) {
      console.error('[DEMO] Fish Audio TTS error:', err.message, '— cannot continue without audio');
      break;
    }
    ...
    // Play TTS audio into VB-CABLE Input.
    // VB-CABLE routes CABLE Input → CABLE Output (the browser's default mic).
    // webkitSpeechRecognition hears it and fires onresult naturally.
    // playToCableInput blocks until the WinMM driver finishes the buffer.
    try {
      playToCableInput(audioBuf);
    } catch (err) {
